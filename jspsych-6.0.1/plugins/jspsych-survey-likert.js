    ...
    plugin.trial = function (display_element, trial) {
        ...
        for (var i = 0; i < trial.questions.length; i++) {
            ...
            for (var j = 0; j < trial.questions[i].labels.length; j++) {
                options_string += '<li style="width:' + width + '%"><input type="radio" class="scale-radio-button" name="Q' + i + '" value="' + j + '"';
        ...
        // disable the submit until a radio button is chosen
        document.getElementById('jspsych-survey-likert-next').disabled = true;
        let radio_buttons = Array.from(document.getElementsByClassName("scale-radio-button"));
        radio_buttons.forEach(b => {
            b.onclick = function () {
                document.getElementById('jspsych-survey-likert-next').disabled = false;
